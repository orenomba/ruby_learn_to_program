...
puts "上のenglishNumberを拡張してみましょう。"
puts "englishNumberにさらに改良を加えましょう。 milion(百万)を加えます。"
class Integer
  def to_english_name
    tmp = self
    words = []
    ...
    words.compact.reject(&:blank?).join " "
  end
...
def englishNumber number
  ...
  words = words.compact.reject(&:blank?).join(" ").strip
end
